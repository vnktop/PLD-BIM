...
namespace BIM.DataAccess
{
    public class ListaNegraDataAccess
    {
        ...
        {
            List<TipoLista> results = new List<TipoLista>();
            try
            {
                using (ListaNegraDataContext dc = new ListaNegraDataContext(Helper.ConnectionString()))
                {
                    var query = from item in dc.stp_ListarTipoLista()
                                select new TipoLista
                                {
                                    strTipoLista = item.vchTipoLista
                                };
                    results.AddRange(query);
                }
            }
        ...
        public bool InsertarTipoLista(List<TipoListaCat> datosTipoLista)
        {           
            int? _id = 0;
            try
            {
                foreach (TipoListaCat item in datosTipoLista)
                {
                     using (ListaNegraDataContext dc = new ListaNegraDataContext(Helper.ConnectionString()))
                    {
                        dc.stp_InsertarTipoLista(ref _id, item.vchTipoLista, item.intTipoTipificacionID, item.VCDDESCRIPCION, item.vchPais);
                        if (_id == 0)
                            return false;
                    }
                }
