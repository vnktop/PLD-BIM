...
using BIM.PLD.Website.ListaNegraService;
...
namespace BIM.PLD.Website.ListasNegra
{
    public partial class frmListaNegra : System.Web.UI.Page
    {
            ...
            try
            {
                if (ddlProveedores.SelectedIndex > 0)
                {
                    if (fupArchivo.HasFile)
                    {
                        ...
                        {
                            bool _res = _validacionTipoLista(_listaNegraArchivo);
                            if (_res == true)
                            {
                                EscribeServidor();
                            }    
        ...
        protected void ingresarNuevosTipos(object sender, EventArgs e)
        {
            ServiceResult _res = new ServiceResult();

            try
            {
                List<TipoListaCat> _listaTipoLista = new List<TipoListaCat>();

                foreach (GridViewRow item in gvdTipoLista.Rows)
                {   
                    TipoListaCat datos = new TipoListaCat();
                    Label _lbTipoLista = (Label)item.FindControl("lbTipoLista");
                    datos.vchTipoLista = _lbTipoLista.Text;
                    DropDownList _ddlTipificacion = (DropDownList)item.FindControl("ddlTipificacion");
                    datos.intTipoTipificacionID = Convert.ToInt32(_ddlTipificacion.SelectedItem.Value);
                    TextBox _tbDescripcion = (TextBox)item.FindControl("tbDescripcion");
                    datos.VCDDESCRIPCION = _tbDescripcion.Text;
                    TextBox _tbPais = (TextBox)item.FindControl("tbPais");
                    datos.vchPais = _tbPais.Text;
                    _listaTipoLista.Add(datos);
                }
                _res = rsListaNegra.InsertarTipoLista(_listaTipoLista.ToArray());
                if (_res.ServiceOk == true)
                {
                    EscribeServidor();
                    mpeAgregarTipoLista.Hide();
                }
                else
                {
                    wucMensajeSistema.setMensaje("Error en el ingresar registro de tipo de lista. Se cancela el proceso de ingreso de lista negra", 2);
                }
            }
            catch (Exception ex)
            {
                wucMensajeSistema.setMensaje("Error en el ingresar registro de tipo de lista" + ex.Message, 2);
                throw;
            }
        }

        protected void cancelarIngresolista(object sender, EventArgs e)
        {
            Response.Redirect("~/ListasNegra/frmListaNegra.aspx");
        }
        ...
        private List<TipoLista> _listaNegraArchivo = new List<TipoLista>();
        private StringBuilder sbtxt = new StringBuilder();
        ...
        protected void gvTipoLista_OnRowCommand(object sender, GridViewCommandEventArgs e)
        {
            
        }

        protected void gvTipoLista_RowDataBound(Object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    DropDownList ddl = (DropDownList)e.Row.FindControl("ddlTipificacion");
                    ddl.DataSource = rsCatListaNegra.ListarCatalogoTipoListaNegra();
                    ddl.DataTextField = "vchDescripcion";
                    ddl.DataValueField = "intCatalogoID";
                    ddl.DataBind();                    
                    ddl.Items.Insert(0, new ListItem("SELECCIONE TIPO", "0", true));
                }
        ...
        protected void gvTipoLista_SelectedIndexChanged(Object sender, EventArgs e)
        {
        ...
        private void EscribeServidor2(StringBuilder registros)
        {
            ...
            string strFileName = strArchivoLista;  
        ...
        private void EscribeArchivo(StringBuilder registros)
        {                        
            try
            {               
                System.Text.UTF8Encoding  encoding=new System.Text.UTF8Encoding();
                string dirOrigen = Server.MapPath("~") + strDirTmpLocal;

                if (!Directory.Exists(dirOrigen))
                    Directory.CreateDirectory(dirOrigen);

                File.WriteAllText(dirOrigen + strArchivoLista, registros.ToString(), System.Text.Encoding.GetEncoding(1252));
                string nf = strArchivoLista;
                string directorioDest = Repositorio;
                ...
                else
                {
                    ...
                    wucMensajeSistema.setMensaje("Ocurrio un error en el copiado de archivos contacte al administrador", 2);
                }
            }
        ...
        {
            string location = Server.MapPath("~") + strDirTmpLocal;

            if (!Directory.Exists(location))
                Directory.CreateDirectory(location);

            File.AppendAllText(location + strArchivoLog,
        ...
        private string strArchivoLista
        {
            get
            {
                return ConfigurationManager.AppSettings["ArchivoLista"].ToString();
            }

        }

        private string strArchivoLog
        {
            get
            {
                return ConfigurationManager.AppSettings["ArchivoLog"].ToString();
            }

        }

        //Archivotmpsbtxt
        private string strArchivotmpsbtxt
        {
            get
            {
                return ConfigurationManager.AppSettings["Archivotmpsbtxt"].ToString();
            }
